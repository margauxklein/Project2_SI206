...
# Requires you to have a twitter_info file in this directory
...
def find_urls(myString):
	#match = re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', myString)
	#match = re.findall(r'(https?://[^\s]+)', myString)
...
def get_umsi_data():
	...
	if 'umsi_directory_data' in CACHE_DICTION:
		print("Using cache for directory")
		return CACHE_DICTION['umsi_directory_data']

	else:
		print("Using live data for directory")
		for x in range(0, 12):
			unique_identifier = requests.get(current_url, headers = {'User-Agent': 'SI_CLASS'})
			unique_identifier = unique_identifier.text
			soup = BeautifulSoup(unique_identifier, 'html.parser')
			#CACHE_DICTION["umsi_directory_data"].append(unique_identifier)
			html_strs.append(unique_identifier) 
			CACHE_DICTION['umsi_directory_data'] = html_strs
			i += 1 
			current_url = base_url + str(i)	
...
def get_five_tweets(phrase):
	...
	if unique_identifier in CACHE_DICTION: # if it is...
		...
		twitter_results = public_tweets['statuses']
		...
		cache_file.close()
	five_tweets = []
	for tweet in twitter_results[:5]:
